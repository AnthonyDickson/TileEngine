...
namespace EconSimPlusPlus {
    namespace DeadReckoningAlgorithm {
        std::vector<std::uint8_t> createSDF(const std::uint8_t* bitmap, const glm::ivec2 bitmapSize,
            ...
            stbir_resize_uint8_linear(sdfImage.data(), paddedSize.x, paddedSize.y, 0, resizedSDFImage.data(),
        ...
        std::vector<float> sdf(const std::uint8_t* buffer, const glm::ivec2 bufferSize, const float distanceAdjacent,
            ...
            for (int y = 0; y < bufferSize.y; ++y) {
                for (int x = 0; x < bufferSize.x; ++x) {
                    if (const auto center{I(x, y)}; I(x - 1, y) != center or I(x + 1, y) != center or
                        I(x, y - 1) != center or I(x, y + 1) != center) {
                        setd(x, y, 0.0f);
                        setp(x, y, {x, y});
                    }
                    else {
        ...
        std::vector<std::uint8_t> createImage(const std::vector<float>& sdf, const float spread) {
